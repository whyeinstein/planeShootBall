 ...
 * 此函数为飞机1躲避飞机2的子弹
 */
int pvp_ai1_moveNeeded2(DataStruct *data, int &des_x, int &des_y, int preTime = 200)
{
    float min_t = INT_MAX;
    //预测每一个子弹的移动
    ...
    {
        //获取当前子弹的引用
        BulletStruct &bullet2 = data->bullet2[i];
        //获取当前子弹的坐标和速度
        float bullet2_x = bullet2.x, bullet2_y = bullet2.y, bullet2_v_x = bullet2.v_x, bullet2_v_y = bullet2.v_y;
        //计算子弹运动直线方程
        float A, B, C;
        A = -1;
        B = bullet2_v_y / bullet2_v_x;
        C = bullet2_y - bullet2_x * bullet2_v_y / bullet2_v_x;
        //计算飞机到直线距离
        float dis = fabs((A * data->plane1.y + B * data->plane1.x + C) / pvp_ai1_vecMod(A, B));
        //计算垂点坐标
        float cross_x = (data->plane1.y - bullet2_y + bullet2_v_y / bullet2_v_x * bullet2_x + bullet2_v_x / bullet2_v_y * data->plane1.x)
                / (bullet2_v_x / bullet2_v_y + bullet2_v_y / bullet2_v_x);
        float cross_y = (bullet2_v_y / bullet2_v_x) * (cross_x - bullet2_x) + bullet2_y;
        //计算到垂点的时间
        float t = (cross_x - bullet2_x) / bullet2_v_x * 100;
        //反向运动或时间过久则忽略该球
        if (t < 0 || t > preTime)
        {
            continue;
        ...
        //判断该球是否有威胁
        if (dis < data->plane1.r + bullet2.r + 10 && t < min_t)
        {
            //设置最紧迫威胁
            min_t = t;
            //向相反方向移动球半径的距离
            des_x = data->plane1.x +  bullet2.r * (data->plane1.x - cross_x)
                    / pvp_ai1_vecMod(data->plane1.x - cross_x, data->plane1.y - cross_y);
            des_y = data->plane1.y +  bullet2.r * (data->plane1.y - cross_y)
                    / pvp_ai1_vecMod(data->plane1.x - cross_x, data->plane1.y - cross_y);
            //如果x越界
            if (des_x < data->plane1.r || des_x > 2000 - data->plane1.r)
            {
                des_x = data->plane1.x;
                if (data->plane1.y - cross_y > 0)
                    des_y = cross_y + bullet2.r + data->plane1.r;
                else
                    des_y = cross_y - bullet2.r - data->plane1.r;
            }
            //如果y越界
            if (des_y < data->plane1.r || des_y > 1500 - data->plane1.r)
            {
                des_y = data->plane1.y;
                if (data->plane1.x - cross_x > 0)
                    des_x = cross_x + bullet2.r + data->plane1.r;
                else
                    des_x = cross_x - bullet2.r - data->plane1.r;
            }
        }
...
void ai_pvp_ai1(DataStruct*data,KeyStruct*key){
    ...
    //标志为0则正在移动或检查发现需要移动
    if (pvp_ai1_moveNeeded(data, des_x, des_y) || pvp_ai1_moveNeeded2(data, des_x, des_y) || moveFlag == 0)
    {
        //进行移动并返回移动结果
