...
//int shieldNeeded(DataStruct *data,int preTime = 250)
//{
...
//        float dis = fabs((A * data->plane1.y + B * data->plane1.x + C) / vecMod(A, B));
//        //无法躲避就直接让飞机撞毁
//        if(dis<data->plane1.r+ball.r+1&&(data->plane1.v_y/data->plane1.v_x)==-(ball_v_y/ball_v_x))
//        {
//            return 0;
//        }
//        //计算垂点坐标
...
//        }
//        //判断该球是否有威胁
//        if (dis < data->plane1.r + ball.r+3&&t<min_t)
//        {
//               return 1;
...
int AimBall(DataStruct *data, BallStruct &ball, int &leftward, int &rightward)
{
    ...
    //预测小球的轨迹（补充）
    preBallPos(ball, 2000, ball_x, ball_y);
    ...
    for (int i = 0; i < data->ball_size; i++)
    {
        ...
        if (dis < data->plane1.r + ball.r + 10 && t < min_t)
        {
            ...
            */
            ...
            if (des_x < data->plane1.r || des_x > 2000 - data->plane1.r)
            {
                des_x = data->plane1.x - 2 *  ball.r * (data->plane1.x - cross_x)
                        / vecMod(data->plane1.x - cross_x, data->plane1.y - cross_y);
            }
            if (des_y < data->plane1.r || des_y > 1500 - data->plane1.r)
            {
                des_y = data->plane1.y - 2 *  ball.r * (data->plane1.y - cross_y)
                        / vecMod(data->plane1.x - cross_x, data->plane1.y - cross_y);
            }
...
int moveNeeded2(DataStruct *data, int &des_x, int &des_y, int preTime = 200)
{
    ...
    {
        BallStruct *ball = &data->ball[i];
        if(nearBallPos(ball, 300))
        {

            float ball_x,ball_y,ball_v_x,ball_v_y;
            //预测球的坐标和速度
            if(nearBallPos(ball, 300)==1)
            {
                ball_x = ball->x, ball_y = ball->y, ball_v_x = - ball->v_x, ball_v_y = ball->v_y;
            }
            else if(nearBallPos(ball, 300)==2)
            {
                ball_x = ball->x, ball_y = ball->y, ball_v_x = - ball->v_x, ball_v_y = ball->v_y;
            }
            else if(nearBallPos(ball, 300)==3)
            {
                ball_x = ball->x, ball_y = ball->y, ball_v_x = ball->v_x, ball_v_y = - ball->v_y;
            }
            else
